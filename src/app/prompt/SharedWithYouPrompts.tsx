...
import { z } from "zod";
import { useGetSharedWithYouPrompts } from "../../api/prompt/useGetSharedWithYouPrompts";
...
const SharedWithYouPrompts = () => {
  ...
  const { data: prompts, isPending: isPromptsLoading } =
    useGetSharedWithYouPrompts(promptsOptions);
      ...
      <div className="mt-5 flex w-full flex-1 flex-col overflow-hidden p-1">
        ...
        <div className="scrollbar mt-5 flex w-full flex-1 flex-col overflow-y-auto pb-5">
          ...
          {!isPromptsLoading && prompts && prompts.items.length > 0 && (
            <>
              <div className="mx-auto flex w-full max-w-2xl flex-1 flex-col space-y-5">
                {prompts.items.map((prompt, idx) => (
                  <button
                    ...
                    className="group flex cursor-pointer items-start justify-start gap-3 rounded-xl border border-gray-300 p-3 text-left dark:border-white/10"
                  >
                    <div className="flex w-full items-start justify-between gap-3">
                      <div className="flex items-start justify-start gap-3 truncate">
                        <div className="bg-primary-700/10 text-primary-500 rounded-xl p-2">
                          <svg
                            viewBox="0 0 24 24"
                            className="size-5"
                            fill="none"
                        ...
                        <div className="flex w-full flex-col truncate">
                          <h3 className="w-full truncate text-base font-medium text-gray-800 group-hover:underline dark:text-white">
                            {prompt.name}
                          </h3>

                          <p className="mt-1 line-clamp-3 text-xs whitespace-normal text-gray-600 dark:text-white/60">
                            {prompt.prompt}
                          </p>
                        </div>
                      </div>
...
export default SharedWithYouPrompts;
