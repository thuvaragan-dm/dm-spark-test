...
import { useTriggerGetPrompts } from "../api/prompt/useTriggerGetPrompts";
...
const RootLayout = () => {
  ...
  const [prompts, setPrompts] = useState<PaginatedResult<Prompt> | null>(null);

  const { mutateAsync: getPrompts } = useTriggerGetPrompts();
  ...
  const askPromptOptions = useMemo<ComboboxResult[]>(() => {
    if (prompts && prompts.items.length > 0) {
      return prompts.items.map((prompt) => ({
        ...
        onClick: () => {
          navigate("/home/chat?set-prompt=true");
          setTimeout(() => {
  ...
  const filteredSearchOptions = useMemo(() => {
    ...
    if (query) {
      ...
      if (filtered.length > 0) {
        ...
        // If no results are found, return only the "Ask" option
        return [
          {
            id: "ask",
            name: `Ask: ${query}`,
    ...
    // If the query is empty, show recents first, without duplicates.
