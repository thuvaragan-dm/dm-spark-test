...
import { z } from "zod";
import { ConnectedMCPServersParams } from "../../api/mcp/types";
...
const AllConnectedMCPServers = () => {
  const [searchQuery, setSearchQuery] = useState("");
  const [page, setPage] = useState(1);
  const [records_per_page, _setRecords_per_page] = useState(20);

  const connectedMCPServersOptions = useMemo<ConnectedMCPServersParams>(
    () => ({
      search: searchQuery,
      page,
  ...
  const { data: connections, isPending: isConnectionsLoading } =
    useGetConnectedMCPServers(connectedMCPServersOptions);
...
export default AllConnectedMCPServers;
